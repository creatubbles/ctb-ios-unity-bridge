...
#include "CreatubblesAPIClient/CreatubblesAPIClient-Swift.h"
...
{
    APIClientSettings *settings = nil;

    const BOOL productionServer = YES;
    if (productionServer)
    {
        // Production
        NSString *const appId = @"";
        NSString *const appSecret = @"";
        NSString *const backgroundSessionConfigurationIdentifier = @"com.creatubbles.explorer.backgroundSessionUpload";

        NSAssert(appId.length > 0, @"Please provide application identifier for your app.");
        NSAssert(appSecret.length > 0, @"Please provide application secret for your app.");
        settings = [[APIClientSettings alloc] initWithAppId:appId
                                                  appSecret:appSecret
                   backgroundSessionConfigurationIdentifier:backgroundSessionConfigurationIdentifier];
    }
    else
    {
        // Staging
        NSString *const appId = @"";
        NSString *const appSecret = @"";
        NSString *const tokenUri = @"https://api.staging.creatubbles.com/v2/oauth/token";
        NSString *const authorizeUri = @"https://api.staging.creatubbles.com/v2/oauth/token";
        NSString *const baseUrl = @"https://api.staging.creatubbles.com";
    ...
    return [[APIClient alloc] initWithSettings:settings];
}
...
- (void)testAPIClient
{
    ...
    [self.apiClient _login:username password:password completion:^(NSError * _Nullable) {
        [self testGetLandingURL];
        [self testGetGalleries];
    }];
}

- (void)testGetLandingURL
{
    [self.apiClient _getLandingURL:LandingURLTypeForgotPassword completion:^(NSArray<LandingURL *> * _Nullable urls, NSError * _Nullable error) {
        if (error)
        {
        ...
        {
            for (LandingURL *url in urls)
            {
                NSLog(@"landingURL: %@ (type=%@)", url.destination, @(url.type));
            }
        }
    }];
}
...
{
    [self.apiClient _getGalleries:nil pagingData:nil sort:SortOrderRecent completion:^(NSArray<Gallery *> * _Nullable galleries, PagingInfo * _Nullable, NSError * _Nullable error) {
        if (error)
        {
            NSLog(@"error: %@", error.localizedDescription);
        }
        else
        {
            ...
            {
                NSLog(@"- %@ (%@)", gallery.name, gallery.identifier);
            }
        }
    }];
}
